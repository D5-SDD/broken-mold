...
export function startUDPListen(cb) {
  if (UDP !== null) {
    console.error('ERROR: UDP socket already in use!');
    return;
  }
  ...
  UDP.socket.on('message', (message, remote) => {
    if (String(message).substring(0,UDP_DM_MSG.length) === UDP_DM_MSG) {
      startTCPClient(remote.address, true, cb);
      stopUDPListen();
    }
...
export function startTCPServer(func, charactersToShare, func2, DM) {
  if (TCP !== null) {
  ...
  TCP.server = net.createServer((c) => {
    ...
    c.on('data', (data) => {
      ...
      try {
        fs.writeFileSync(DM_FOLDER + temp2.name + '.json', temp[1]);
      } catch (e) {
        console.log('Error in writing file');
      }
      func(temp2.name + '.json', c);
...
export function startTCPClient(address, DM, cb) {
  console.log('start');
  if (TCP !== null) {
    console.error('ERROR: TCP socket already in use!');
    return;
