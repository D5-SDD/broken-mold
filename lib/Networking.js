...
export function startTCPServer(func, charactersToShare) {
  ...
  TCP.server = net.createServer((c) => {
    ...
    c.on('data', (data) => {
      ...
      try {
        fs.writeFileSync(CHAR_LOCATION + temp.name + ' - DMTemp.json', data);
      } catch (e) {
        console.log('Error in writing file');
      }
      func(temp.name + ' - DMTemp.json');
...
export function startTCPClient(address, DM) {
  ...
  TCP.client.on('data', (data) => {
    if (DM) {
      ...
      var temp = JSON.parse(data);
      try {
        fs.writeFileSync(CHAR_LOCATION + temp.name + ' - shared.json', data);
      } catch (e) {
        console.log('Error in writing file');
      }
    }
