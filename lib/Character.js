...
const SPELLS_DB = JSON.parse(fs.readFileSync('./lib/db/spellsDB.json'));  // spells database
...
export function exportMap(characterDirectory) {
  var map = [];
  var characters = [];
  
  var characterFS = require('fs');
  var characterFiles = characterFS.readdirSync(CHARACTER_DIR);
  
  for (var f in characterFiles) {
    var fileLen = f.length;
    var extension = f.substring(n-6);
    if (extension === '.json') {
      var c = JSON.parse(characterFS.readfileSync(CHARACTER_DIR + f));
      characters.push(c);
    }
  }
  ...
  for (let i = 0; i < characters.length; i++) {
    ...
    charToExport.label = charToExport.name + ' - ' + charToExport.race;
    for (let i = 0; i < charToExport.classes.length; i++) {
      charToExport.label += ' - ' + charToExport.classes[i].name + ' ' + charToExport.classes[i].level;
    }
...
export function loadCharacters(paths) {
  try {
    ...
    console.log('Character could not be made');
    return null;
  }
}
