...
/* 
 * Character.js:
 * This file stores all the information about 
 * a D&D character and functions for accessing
 * and calculating certain information
 */

//Used for accessing the file system
import fs from 'fs';

//path to character map
const CHARACTER_MAP_PATH = './test/character_map.json';

//location of saved characters
const CHARACTER_DIR = './test/Characters/';

//location of spells database
const SPELLS_DB = JSON.parse(fs.readFileSync('./lib/db/spellsDB.json'));

//The list of skills organized by ability they are associated with
export const SKILLS = {
  strength: [
    'athletics'
  ],
...
/*
 * Character
 * Class that contains the information for a D&D character.
 * Also contains the functions for saving and reading in a character
 * from a JSON file.
 */
export default class Character {
  /*
   * Constructor to determine if we are loading
   * a JSON file or creating a new character
   *
   * path: path to the JSON file if loading
   *   or 'NEW_CHARACTER' if a new character 
   *   is being created
   */
  constructor(path) {
  ...
  getListOfSpells(spellClasses, spellLevel) {
    ...
    for (let i = 0; i < SPELLS_DB.length; i++) {
      //If we match one class and the spell level,
  ...
  /*
   * Reads in a character from a JSON file
   * and creates the js object for that character.
   * Any auto-calculated value is also calculated 
   * during import.
   *
  ...
  /*
   * Calculates the label used on the
  ...
  /*
   * Calculates the ability score mods
   */
  ...
  /*
   * Calculates the values for the skills
   */
  calculateSkills(){
  ...
  /*
   * Calculates the spell save DC and
   ...
   * (the sum of all your classes level)
   */
  ...
  /*
   * Recalculates all the auto calculated values
   * for the character
   */
  updateAutoValues() {
  ...
  /*
   * Saves all the essential information in the js
   * character object and store it in a JSON file
   */
  saveCharacter() {
    var char = {};
    char.name = this.name;
  ...
  /*
   * Checks if a character is saveable
   */
  isCharacterValid() {
  ...
  /*
   * Sets up a character with no information
   * Essentially, a base character whose information
   * needs to be added
   */
...
/*
 * A function to wrtie a js object to JSON
 *
 * path: the file path to the JSON file we are writting to
 * object: the js object to turn into a JSON file
 */
...
/*
 * Reads the characters from the character map
 */
...
/*
 * Creates the charater map from an array of character
 *
 * characters: an array of characters to make a map from
 */
